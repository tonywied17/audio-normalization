...
class AudioProcessor:
    ...
    @staticmethod
    def _update_audio_track_title(original_title, operation, extra_info=""):
        """
    ...
    def normalize_audio(self, media_path, temp_files):
        """
        ...
        try:
            ...
            self.logger.info("Analyzing loudness for audio normalization.\n\n[bold]Audio tracks:[/bold] " + str(len(audio_streams)))
            loudness_metadata = []
            ...
            for i, audio_stream in enumerate(audio_streams):
                ...
                new_title = self._update_audio_track_title(original_title, "Normalized")
            ...
            if os.path.exists(final_output_path):
                self.logger.info(f"File exists: {final_output_path}. Removing it before replacement.")
                os.remove(final_output_path)
            os.rename(temp_output_path, final_output_path)

            self.logger.success(f"Normalization completed.\n[bold]File:[/bold] {final_output_path}")
    ...
    def filter_audio(self, media_path, volume_boost_percentage, temp_files):
        """
        ...
        try:
            ...
            if os.path.exists(media_path):
                self.logger.info(f"File exists: {media_path}. Removing it before replacement.")
                os.remove(media_path)
            os.rename(temp_output_path, media_path)

            self.logger.success(f"Volume adjustment completed.\n\n[bold]Adjustment:[/bold] {volume_boost_percentage}%\n[bold]File:[/bold] {media_path}")
            return True
            ...
            self.logger.error(f"Volume adjustment failed.\n[bold]File:[/bold] {media_path}\nError: {e}")
            return False
